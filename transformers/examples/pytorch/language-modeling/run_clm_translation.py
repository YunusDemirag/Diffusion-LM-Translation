...
def get_corpus_wit3(data_args):
    ...
    valid_sentence_lst = []
    with open(f'{data_args.valid_file}') as valid_data_file:
        lines = train_data_file.readlines()
    for line in lines:
        words_in_line = [token.text for token in tokenizer(line)]
    ...
    raw_datasets = DatasetDict({'train': train_dataset, 'test': valid_dataset})
...
def main():
    ...
    ### Translation Experiment ###
    elif model_args.experiment.startswith('roc') and model_args.task == 'translation':
        raw_datasets = get_corpus_wit3(model_args)
    ...
    if model_args.model_name_or_path:
        ...
        elif model_args.experiment in ['synth_emb', 'pos_emb', 'roc_emb', 'simple-wiki_emb', 'e2e-tgt_emb']:
